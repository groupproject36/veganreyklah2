...
- **The gate trio runs** — `parity.rish` (behavior), `parity-selftest.rish` (proves RED on divergence), `additive-gate.rish` (shape). All three run in Rishi; sixty-six witnesses, all green.
...
- **The parity gate runs in Rishi** — `tools/parity.rish` runs each witness `.rye` through `rye run` (baseline via `RYE_LIB=vendor/zig-toolchain/lib`, strengthened via default `rye/lib`), compares outputs as one value, and asserts the verdict. Proven GREEN across 66 programs and RED on tamper (`parity-selftest.rish`).
...
- **Strengthening passes 9979–9923** — `trimStart` … mem.toBytes. Each through the gate; witness count 17 → 80.
...
- [x] **Strengthening 9979–9924** — everyday `std` surfaces through the gate (`trimStart` … mem.bytesAsSlice; see `strengthening-compiler/9924`–`9987`).
...
1. **Strengthening series** — next `std` surface (`9923` and below; `998`) — invoke with **`k <stamp>`**
...
5. **Caravan capability table** — `984` step 5 + **RyeVersion** when needed
6. **Tablecloth v1** — after 1–2 stay green (`10022`)

**Braid:** `k` runs strengthening; between them, one width module from the Phase 1 queue. Aurora metal smoke when the main mem arc pauses.
...
Tally v1 gardens, Brix minimum, Mantra repo chain, Rishi gate trio, Caravan chain, Brushstroke Wayland, Comlink hosted wire, Skate glyph grid, path naming family, garden-memory policy, **TAME document stack (`014512`)**, **SemanticVersion** trio, **mem** compare/copy/sentinel arc, **timing_safe.eql (9937)** on the metal path, **Keccak/SHAKE `@memcpy` (9936 + 9935)** on the metal path, mem iterators through **WindowIterator (9954)**, mem.join (9961), cut family (9958–9960), **split factories (9934 + 9933)**, **tokenize factories (9932)**, **window factory (9931)**, **mem.trim (9930)**, **crypto.secureZero (9929)**, **mem.span (9928)**, **ShakeLike.update (9927)**, **mem.sliceAsBytes (9926)**, **mem.asBytes (9925)**, **mem.bytesAsSlice (9924)**, **Tally explicit widths (`211712`)**, **`caravan/seed` widths (`014512`)**, **`caravan/bounded` widths (`015712`)**, **`caravan/twin` widths (`020512`)** — done. **Strengthening (`9923`)** via `k`; **width Phase 1** continues `caravan/chain` → Skate between `k` runs. **Freestanding smoke** on Aurora metal lane; **RyeVersion** beside capability table.

---

## The Steps Just Taken

Strengthening through **9924**, explicit-width Tally Phase 1 on `u32`, parity **79/79** green.

**Strengthening passes 9979–9924** (`143312`–`022612`): witness count 17 → 79, all green.
