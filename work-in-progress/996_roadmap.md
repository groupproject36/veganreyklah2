...
- **The gate trio runs** — `parity.rish` (behavior), `parity-selftest.rish` (proves RED on divergence), `additive-gate.rish` (shape). All three run in Rishi; sixty witnesses, all green.
...
- **The parity gate runs in Rishi** — `tools/parity.rish` runs each witness `.rye` through `rye run` (baseline via `RYE_LIB=vendor/zig-toolchain/lib`, strengthened via default `rye/lib`), compares outputs as one value, and asserts the verdict. Proven GREEN across 59 programs and RED on tamper (`parity-selftest.rish`).
...
- **Rishi string builtins** — `length`, `trim`, `slice`, `lines`, `starts-with`, `ends-with`, `split`, `join`, `contains`, infix `index-of`; gate trio 59/59 GREEN.
- **Skate text grid** — monospace 8×8 glyphs on Brushstroke Wayland seed; headless selftest green (`10023` Track A).
- **Strengthening passes 9979–9944** — `trimStart` … `mem.copyBackwards`. Each through the gate; witness count 17 → 59.
...
- [x] **Strengthening 9979–9944** — everyday `std` surfaces through the gate (`trimStart` … `mem.copyBackwards`; see `strengthening-compiler/9944`–`9987`).
...
1. **Strengthening series** — next `std` surface through the gate trio (`9943` and below; `998`)
...
Tally v1 gardens, Brix minimum, Mantra repo chain, Rishi gate trio, Caravan chain, Brushstroke Wayland, Comlink hosted wire, Skate glyph grid, path naming family, garden-memory policy, **SemanticVersion** trio (**format 9946**, **parse 9947**, **order 9948**), **mem.sliceTo (9945)**, **copyBackwards (9944)** and **copyForwards (9943)**, mem compare/count through **findDiff (9949)** onward, mem iterators through **WindowIterator (9954)**, mem.join (9961), cut family (9958–9960) — all done. **Strengthening the next `std` surface (9942)** stays the honest next step on the main track; **RyeVersion** accretion waits beside the capability table.

---

## The Steps Just Taken

Strengthening through **9944**, garden-memory policy in TAME and rules, Rishi builtins and parser fix, Skate grid, parity via `rye run`, witnesses vocabulary, style audit shipped.

**Strengthening passes 9979–9944** (`143312`–`194012`): `trimStart` … `mem.copyBackwards`. Witness count 17 → 59, all green.
