...
- **Strengthening passes 9979–9916** — `trimStart` … mem.replacementSize. Each through the gate; witness count 17 → 87.
...
- [x] **Strengthening 9979–9916** — everyday `std` surfaces through the gate (`trimStart` … mem.replacementSize; see `strengthening-compiler/9916`–`9987`).
...
1. **Strengthening series** — next `std` surface (`9915` and below; `998`) — invoke with **`k <stamp>`**; each pass includes width audit on affected files
