...
Three modules remain entirely in design: Silo (the configuration language), Mantra (the content-addressed memory), and Pond (the enclosure that composes Caravan and Tally). Per Gall's Law, none of them should start until their prerequisites run. Silo waits on a mature Rye to describe; Mantra waits on a mature value model and network; Pond waits on Caravan and Tally v1. The design placed them in that order deliberately, and the code has honored it.
