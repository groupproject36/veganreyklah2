...
**Voice:** Reya 2
**Lens:** TAME Style (`996_TAME_STYLE.md`); active-designing principles (Gall's Law, one value model, simplicity, the gates)
**Reads alongside:** `995_ownerboot_riscv_caravan.md`, `994_caravan_tally_networking.md`, `993_aurora_boot_riscv.md`; sources honored in `../gratitude/urbit`, `../gratitude/sui`
...
- **Rye types the packet.** A datagram is not a smear of bytes but a value with a shape: sender identity, receiver identity, a nonce, a content hash, a payload. The encryption (a Diffie–Hellman key between owner keys, an authenticated cipher over the payload) is a function on that value, asserted — key present, nonce never repeated, length within bounds — in the strengthened `std` we are already tending.
