...
- **A post is a Rye value, signed and named by Mantra.** Not a JSON blob marshaled across a seam, but a typed value whose content hash (SHA3-512, the function we strengthened first) is its name, signed by the owner key. A reply references the hash it answers; a thread is a small DAG of named values, gaps visible rather than silent.
...
- **The feed is a value Silo describes and you can inspect.** Ranking is by honest relations — chronology, explicit follows, a web of trust among keys — declared as configuration, never a hidden engine. Primal's lesson (curate the firehose into feeds) without Primal's scale or opacity: the curation function is small, readable, and yours to change.
...
Further out, the social layer is simply our one value model worn as community: a note is the same kind of value as a Mantra line, a Silo build, a packet in `985` — signed, named, bounded, owned — and the relay is the same kind of service as any Caravan supervises. A person could run their whole presence inside a Pond on their own RISC-V machine: identity in the owner key, words named by Mantra, a personal relay bounded by Tally, a feed they wrote in Rishi and can read in full. The commons becomes a graph of named, signed values among friends — no owner, no engine, no mining — curated by trust made visible. That is the quiet ambition: not another platform, but a place that behaves the way the values do.
