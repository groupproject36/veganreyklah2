...
**Ring 1 landed** (`2026-06-30`): value as explicit `[]const Line` input; arbitrary-value witness GREEN. **Ring 2 landed** (`2026-06-30`): `setFrame` re-folds and re-presents; FNV content-signature witness GREEN (A→B→A); `redraw` metal mode confirmed. **Ring 3 witness landed** (`2026-07-02`): live session mirrored headlessly; parity **140**; `pond/apps/drawn_terminal.rye`; `metalsmoke` for automated Wayland slice — interactive idle/ceiling on GNOME still manual. **Ring 3 metal close** — Kaeden confirms on GNOME. **SLC-2b next** — Dexter; bench prompt [`20260702-030112_cursor-slc-2b-keyboard-from-window.md`](../expanding-prompts/20260702-030112_cursor-slc-2b-keyboard-from-window.md) placed, not built.
